...
<script>
	...
	function search() {
		var hh = $("#hh").val();
		$.ajax({
			url : 'ystrade_list_deal.jsp', //后台处理程序			
			type : 'post', //数据发送方式
			data : {
				action : "search",
				hh : hh,
	...
	function showResult(json) {
		//清空显示数据
		$("#ystrade_list_table tr").eq(0).nextAll().remove();
		$("#pagecount").empty();
		...
		$.each(
				...
				function(index, array) { //遍历json数据列  
					...
					tr += "<tr bgColor=" + bgColor + "><td>"
							+ array['id'] + "</td>" + "<td>"
							+ array['tradeNo'] + "</td>" + "<td>"
							+ array['nickName'] + "</td>" + "<td>"
							+ array['customerName'] + "</td>" + "<td>"
							+ array['createTime'] + "</td>" +"<td>"
							+ "</tr>";
				});
		$("#ystrade_list_table").append(tr);
	}
...
<TABLE class=table_southidc cellSpacing=1 cellPadding=2 width=800
	border=0>
	<TBODY>
		<TR class=tr_southidc>

			<TD>
				<TABLE id="ystrade_list_table" width="100%" align=center border=0 border=0>
					<TBODY>
						<TR bgColor=#a4b6d7>
							...
							<TD width="20%">
								<DIV align=center>原始单号</DIV>
							</TD>
							<TD width="25%">
								<DIV align=center>客户网名</DIV>
							</TD>
							<TD width="25%">
								<DIV align=center>收件人姓名</DIV>
							</TD>
							<TD width="25%">
