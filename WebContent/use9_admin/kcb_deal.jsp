...
<%
	...
	if (request.getParameter("action") != null) {//查询
		if (request.getParameter("action").equals("query")) {
			...
			String hql = "from ApiKcb as api_kcb order by apiTime desc";
			...
			ApiKcb apiKcb = (ApiKcb) session1
					.get(ApiKcb.class, taskId);
			apiKcb.setZt(0);
			apiKcb.setUpdateTime(new Date());
			try {
				session1.update(apiKcb);
			...
			SessionFactory sf = config1.configure().buildSessionFactory();
			Session session1 = sf.openSession();
			String hql = "from ApiKcb as api_kcb where hh =:hh";
			Query query = session1.createQuery(hql);
			...
			Session session1 = sf.openSession();
			session1.getTransaction().begin();
			String hql = "update ApiKcb set zt = 0,updateTime= :newDate where zt =1 and hk !=0";
			Query query = session1.createQuery(hql);
