...
<script>
	...
	function showResult(json) {
		//清空显示数据
		$("#kcb_table tr").eq(0).nextAll().remove();
		$("#pagecount").empty();
	...
	$("body").on("click", ".btnStatus", function() {
		...
		$.ajax({
			...
			success : function(json) {
				if (json.code == 0) {
					curTr.find("td").eq(5).text("未提交");
					curTr.find("td").eq(4).text(json.update_time);
					curTr.find("td").eq(9).find("button").attr('disabled','disabled');
				} else {
					alert('修改状态失败');
				}
			},
...
<TABLE class=table_southidc cellSpacing=1 cellPadding=2 width=1000
	...
	<TBODY>
		<TR class=tr_southidc>
			...
			<TD>
				<TABLE id="kcb_table" width="100%" align=center border=0 border=0>
					<TBODY>
						<TR bgColor=#a4b6d7>
							...
							<TD width="10%">
								<DIV align=center>仓库编号</DIV>
							</TD>
							<TD width="10%">
								<DIV align=center>货号</DIV>
							</TD>
							<TD width="10%">
								<DIV align=center>库存数</DIV>
							</TD>
							<TD width="15%">
